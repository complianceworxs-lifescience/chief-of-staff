...
const PORT = process.env.EXP_PORT || 3001;
...
app.post("/xp/event", (req, res) => {
  ...
  // views already counted via /assign sessions
...
app.get("/xp/report", requireAdmin, (req, res) => {
  ...
  const post = exp.post || {};
  const summary = Object.fromEntries(Object.entries(post).map(([k, v]) => [
    k,
    {
      sessions: v.sessions,
      clicks: v.clicks,
...
// List experiments
app.get("/xp/overview", requireAdmin, (_req, res) => {
  const exps = readJSON(FILES.exps);
  res.json(exps.map(e => ({ id: e.id, page: e.page_slug, created_at: e.created_at, variants: Object.keys(e.variants) })));
});

// Promote winner -> playbook append
app.post("/xp/promote", requireAdmin, (req, res) => {
...
function computeDrift() {
  ...
  const alerts = [];
  for (const [key, v] of Object.entries(byExpVar)) {
...
// Health check for main system integration
app.get("/health", (_req, res) => {
  res.json({ status: "healthy", service: "experiment-os", timestamp: nowISO() });
});

// Minimal UI for CoS (no curl needed)
app.get("/", (_req, res) => {
  res.type("html").send(`
...
async function defineExp(){
  const r = await fetch('/xp/define',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+T},body:document.getElementById('exjson').value});
  document.getElementById('msg1').textContent = (await r.json()).ok?'Saved':'Error';
}
async function overview(){
  const r = await fetch('/xp/overview',{headers:{'Authorization':'Bearer '+T}}); document.getElementById('ov').textContent = JSON.stringify(await r.json(),null,2);
}
async function report(){
  const id=document.getElementById('expid').value; const r=await fetch('/xp/report?exp='+encodeURIComponent(id),{headers:{'Authorization':'Bearer '+T}}); document.getElementById('rep').textContent=JSON.stringify(await r.json(),null,2);
}
async function promote(){
  const id=document.getElementById('expid').value, w=document.getElementById('winner').value;
  const r=await fetch('/xp/promote',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+T},body:JSON.stringify({exp:id,winner:w})});
  alert(JSON.stringify(await r.json(),null,2));
}
async function alerts(){
  const r=await fetch('/alerts',{headers:{'Authorization':'Bearer '+T}}); document.getElementById('al').textContent = JSON.stringify(await r.json(),null,2);
}
</script>
  `);
});

app.listen(PORT, () => console.log(`Experiment OS running on http://localhost:${PORT}`));