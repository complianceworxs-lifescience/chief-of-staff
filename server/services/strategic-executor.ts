...
import { actionTracker, type ActionRecord } from './action-tracker.js';
...
export class StrategyExecutor {
  ...
  /**
   * Submit action through governance system with proper O→D→A→R tracking
   */
  async submitGoalAction(action: OverdueGoalAction): Promise<void> {
    ...
    // OBSERVE → Already done (goal identified as overdue)
    // DECIDE → Classify as Critical and select action
    console.log(`🔍 OBSERVE: Goal "${action.goalTitle}" at overdue status - ${action.priority} priority`);
    console.log(`🎯 DECIDE: Classified as ${action.priority.toUpperCase()} - assigning ${action.actionType} to ${action.assignedAgent.toUpperCase()}`);
    
    // ACT → Create Action Record with full O→D→A→R compliance
    const actionRecord = await actionTracker.createActionRecord(
      `STRATEGIC RECOVERY: ${action.goalTitle}`,
      action.assignedAgent,
      {
        kpi: this.determineKPI(action.goalTitle),
        target: this.determineTarget(action.goalTitle, action.priority),
        time_window_hours: 48,
        scope: `Strategic goal recovery: ${action.goalTitle}`,
        pass_fail_rule: this.determineSuccessRule(action.goalTitle, action.priority),
        canary_size: 15,
        confidence_pct: action.priority === 'high' ? 85 : 75
      },
      action.priority === 'high' ? 'medium' : 'low', // Risk level
      0 // Zero spend for autonomous execution
    );
    ...
    // Legacy governance system integration
    const recommendation = {
      title: `STRATEGIC RECOVERY: ${action.goalTitle}`,
      ...
      spend_cents: 0,
      payload: {
        actionRecordId: actionRecord.action_id,
        goalId: action.goalId,
        autoGenerated: true,
        urgentAction: true,
        ownershipMatrix: this.getOwnershipMatrix()
      },
      rationale: `O→D→A→R: ${action.goalTitle} requires immediate intervention. Action Record: ${actionRecord.action_id}`
    };
    
    await actOnRecommendation(recommendation);
    
    // Update agent assignment
    await this.updateAgentAssignment(action, dueDate);
    
    // REPORT → Action Record created, outcome tracking in 24h
    console.log(`📊 REPORT: Action Record ${actionRecord.action_id} created`);
    console.log(`⏰ OUTCOME DUE: 24h deadline for ${action.assignedAgent} to record results`);
    console.log(`✅ O→D→A→R COMPLETE: ${action.assignedAgent} → "${action.goalTitle}"`);
  }
  
  /**
   * Get ownership matrix per attached directive
   */
  private getOwnershipMatrix(): Record<string, string[]> {
    return {
      'ceo': ['KPI swings >10% vs plan', 'cross-agent conflicts', 'prioritization'],
      'cos': ['governance', 'escalations', 'overdue/outcome gaps', 'weekly rollups'],
      'coo': ['job failures', 'uptime', 'data freshness', 'MTTR', 'incident handling'],
      'cmo': ['activation', 'CTR', 'conversion leaks', 'content underperformance'],
      'cro': ['MRR flat/down', 'churn risk', 'expansion/upsell plays'],
      'cco': ['validation deadlines', 'deviation risk scores', 'audit readiness']
    };
  }
  
  /**
   * Update agent with assignment details
   */
  private async updateAgentAssignment(action: OverdueGoalAction, dueDate: Date): Promise<void> {
    const agentMapping = { 'cro': 'cro', 'cco': 'cco', 'cmo': 'cmo', 'coo': 'coo' };
    const agentId = agentMapping[action.assignedAgent] || action.assignedAgent;
    
    try {
      await storage.updateAgent(agentId, {
        lastReport: `O→D→A→R ASSIGNMENT: ${action.goalTitle} recovery - DUE ${dueDate.toLocaleDateString()}. Primary action: ${action.recommendations[0]}`,
        lastActive: new Date()
      });
    } catch (error) {
      console.log(`⚠️ Agent ${agentId} not in database - assignment tracked in Action Record system`);
    }
  }
